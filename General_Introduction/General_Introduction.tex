...
\boldheader{Leveraging long-reads for isoform annotation}
Significant advances have been made in the realm of long read sequencing of isoforms (\cref{fig:longread_benefits}). With incremental improvements in chemistry, subsequent increase in throughput and diminishing sequencing cost, both PacBio isoform sequencing (Iso-Seq) and Oxford Nanopore sequencing has become more widely used for isoform annotation and characterisation of splicing events (summarised in \cref{tab: longread_isoseqstudies} and \cref{tab: longread_ontstudies} respectively). A common theme across all these studies is the unprecedented widespread isoform diversity with a significant proportion of isoforms identified as novel and not currently annotated in the human \cite{Sharon2013, Au2013,Tseng2019} and mouse transcriptome. Moreover, validation of these novel isoforms with proteomic approaches suggest that some of these novel isoforms with novel splice junctions and splicing events are functionally relevant with biological implications \cite{Huang2021}. 
...
%			Achieved very good correlation with illumina data within the same cells (\cite{Byrne2017})
...
%GGC repeat expansion in the 5'UTR of \textit{NOTCH2NLC} \cite{Sone2019, Deng2019}
...
%Reyes2021 genomic sequencing with oxford nanopore for repeats in other neurodegenerative disorders
...
\begin{changemargin}{1.5cm}
	\begin{landscape}
	\begin{figure}[]
		\hspace*{2cm}\includegraphics[page=1,trim={0 5.5cm 1.8cm 0},clip, scale = 0.8]{Introduction_Figures_Landscape.pdf}
	...
	\begin{landscape}
		...
		\begin{longtable}[c]{p{4cm}p{4cm}p{18cm}}
			\caption[Long-read sequencing studies using Pacific Biocience's Isoform Sequencing]%		
			{\textbf{Long-read sequencing studies using Pacific Biocience's Isoform Sequencing}. ES - Exon skipping, FL - full-length; hiPSC - human induced pluripotent stem cell;HCC - Hepatocellular carcinoma; HCM - Hypertrophic Cardiomyopathy; hESCs - Human embryonic stem cells, ISS - Intronic-Splice Site, TSS - Transcription start site, TTS - Transcription termination site }
			...
			\tabitem High isoform diveristy observed within single-cell oligodendrocytes with \textasciitilde1000 distinct isoforms mapped to 700 genes with low overlap between cells, predominatly driven by alterantive TSS and TSS \\
			\hdashline[0.5pt/5pt]	
			
			\centering Tseng et al. (2017) \cite{Tseng2017} &
			\centering Human cerebellum \newline (3 carriers, 3 controls)  &
			\tabitem Targeted sequencing of \textit{FMR1} in premutation carriers at risk of fragile X syndrome identifid 49 isoforms, with increased expression of novel truncated isoforms in premutation group \\
			\hdashline[0.5pt/5pt]	
			...
			\tabitem Patient-derived hiPSC-neurons (psychosis-diagnosed individuas with rare \textit{NRXN1} heterozygous deletions) displayed aberrant \textit{NRXN1} isoform expression, with downregulation of \textit{NRXN1$\alpha$} owing to reduced abundance of WT isoforms and expression of 31 novel, mutant isoforms with reduced neuronal activity  \\
			\hdashline[0.5pt/5pt]
			...
			\centering Human brain marrow cells  &
			\tabitem  \\
			\hdashline[0.5pt/5pt]
		...
		\begin{longtable}[c]{p{4cm}p{4cm}p{18cm}}
			...
			\tabitem Enrichment of \textit{BRCA1} performed with loong-range RT-PCR resulted in biased amplifcation of short transcripts (<4kB), and many of the novel isoforms had <3 MinION reads (> 10\% error rate)  \\
			\hdashline[0.5pt/5pt]
			...
			\tabitem Ability to accurately demultiplex reads based on 7-8nt barcodes enabling mass sequencing of singple cells with accurate gene quantification (strongly correlated with RNA-Seq, r = 0.79) and identification of cell-specific isoforms \\
			...
			\tabitem Accurately identification of splice variants with no missing or novel exons from spike-in, and able to rudimentally discrimate RNA modification (m\textsuperscript{6}A, 5-mC) using trained datasets  \\
			\hdashline[0.5pt/5pt]
			
			\centering Workman et al. (2019) \cite{Workman2019a} &
			\centering Huamn B lymphocyte cell line (n = 30) &
			\tabitem Direct RNA-Sequencing of human cell line documented a high proportion (52.6\%) of novel isoforms \newline
			\tabitem High error rate (14\%) \& signficant 5'truncation due to technical issues (rapid translocation through pore, signal artefacts from enzyme stalling or strand breaks), making it difficult to ascertain TSS \newline
			...
			\tabitem Targeted sequencing of GWAS neuropsychiatric-associated haplotype blocks containing non-coding SNPs identifed 107 novel intergenic transcripts (novel genes) classed as putative lncRNAs \newline 
			...
			\tabitem Novel sequencing method (RAGE-Seq) combininig droplet-based scRNA-Seq with target capture ONT cDNA-Seq \newline 
			...
			\tabitem Brain-regionial isoform expression differences with notable isoform switch between cerellum and cortex  \\
			\hdashline[0.5pt/5pt]
			...
			\tabitem Novel bioinformatics tool (FLAIR) to correct ONT reads with short reads, identifiyng aberannt 3'SS \& retained intron usage in CLL samples with \textit{SF3B1} mutation; downregulation of intron-retained isoforms containing conserved motif upstream of 3'SS (motif only found using short-read RNA-Seq due to nanopore length bias), enriched in NMD  \\
			\hdashline[0.5pt/5pt]
			
			\centering Tian et al. 2020 \cite{Tian2020} &
			\centering Human cell lines \& PMBCs (1 CLL) \newline Mouse muscle stem cells &
			\tabitem Novel sequencing method and tool (FLT-Seq, FLAIR) combining scRNASeq, ONT cDNA-Seq \& Illumina RNA-Seq \newline 
			\tabitem Sequencined 2,800 single cells, identifiying thousands of cell-specific novel isoforms and differential isoform usage between cell lines for genes enriched in mRNA splicing and cell-surface receptors \newline 
...
\boldheader{Leveraging long-reads for isoform annotation}
Significant advances have been made in the realm of long read sequencing of isoforms (\cref{fig:longread_benefits}).
