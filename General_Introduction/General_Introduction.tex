...
%https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5961815/?report=classic#r10-2835284 --> relevance of SNP in regulatory element affecting spliceosome
...
\begin{landscape}
	...
	\begin{longtable}[c]{p{3cm}p{4cm}p{3cm}p{16cm}}
		...
		\caption{Transcriptomic and proteomic studies in AD human post-mortem brain tissues and mouse models in revealing missplicing as a widespead hallmark of AD}\\
		\toprule
		...
		\endlastfoot
		%
		\centering Tollervey et al. (2011)\cite{Tollervey2011} &
		\centering 9 AD, 43 Controls\newline Middle Temporal Gyrus &
		\centering Microarray, RT-PCR &
		Age-related splicing changes in normal individuals also present in 95\% of AD individuals, and were related to PTB-dependent splicing activity. Occurrence of age-related and disease-specific changes within same transcript (CHL1, NRCAM). Genes with disease-specific changes were enriched in neuronal functions (synpatic transmission, ion channel genes) \\
		\centering Twine et al. (2011)\cite{Twine2011} &
		\centering 3 AD, 33 Controls\newline Total, frontal \& temporal lobe &
		\centering RNA-Seq &
		Downregulation of APOE in AD temporal lobe with observed differential isoform expression:  3 APOE isoforms were detected with different transcription start site: ENST00000252486 and ENST00000446996 from TSSA were downregulated (3.09-fold) whereas ENST00000425718 from TSSB were upregulated (26.5-fold)Downregulation of Ank1 in total brainNote: A number of genes of top 30 differentially expressed genes without annotation from Ensembl \\
		\centering Mills et al. (2013)\cite{Mills2013} &
		\centering 5 AD, 5 Controls \newline Parietal lobe &
		\centering RNA-Seq &
		Identification of differentially expressed genes enriched in lipid metabolism (upregulation of ACOT1 and ACOT2, downregulation of TECR)Upregulation of DBI gene (ACBP) with observed differential isoform expression: upregulation of non-coding DBI-009 accompanied by downregulation of protein-coding DBI-003. DBI-009 and DBI-003 have different transcription start sites. \\
		\centering Bai et al. \newline(2013)\cite{Bai2013} &
		\centering Human: 10 AD, 10 Controls Cortex &
		\centering Mass-Spectrometry &
		36 proteins identified in AD (\textless5\% false discovery rate) from insoluble proteome; tau, AB peptide and APOE with highest spectral counts. Importantly, other RNA splicing factors such as hnRNP A/B suggested as dysfunctional splicing factor in AD and SSRM2 not identified in list of insoluble proteome nor demonstrate tangle-like isolates, indicating U1-snRNP specific process. Levels of subunits highly correlated with Abeta rather than tau, supporting U1 snRNP accumulation specific to AD and early during disease development. \\
	
		\centering Lai et al. (2014)\cite{Lai2014} &
		\centering 8 AD, 8Controls\newline Superior Temporal Gyrus &
		\centering Microarray &
		22 genes identified with differential AS event (characterised by differential exon usage); \textit{GNAL} transcript variant 5 downregulated in AD whereas transcript 1 showed no change, and \textit{MAP4} transcript variant 3 was downregulated in AD whereas transcript variant 1 was upregulated\\

		\centering Mills et al. (2014)\cite{Mills2014} &
		\centering 14 AD, 16 Controls \newline STG &
		\centering RNA-Seq
		&
		APOE alternative transcription is unaffected in STG of AD brains, authors concluding dysregulation of APOE expression is not a common occurrence in AD \\
		\centering Humphries et al.(2015)\cite{Humphries2015} &
		\centering 10 AD, 10 Controls, 10 DLB \newline Temporal lobe &
		\centering RNA-Seq &
		Second paper to report whole transcriptome profiling of hippocampus of late-onset Alzheimer’s disease (LOAD) vs control with expression of both non-coding and coding protein RNA transcripts. (But first to work on hippocampus)After correcting for multiple testing by False Discovery Rate (FDR \textless 0.05), only 16 genes found to be significantly differentially expressed. In further testing that genes are specific to LOAD (and not general neurodegeneration), narrowed down to 5 genes (2 protein-coding, ABCA7, TMEM259, EPHA1, MS4A6A, andMS4A6E) that had differing exon distribution when compared LOAD to disease controls \\
		\centering Magistri et al.(2015)\cite{Magistri2015} &
		\centering 4 AD,4 Controls \newline Hippocampus &
		\centering RNA-Seq &
		Downregulation of \textit{TAC1} and upregulation of \textit{SERPINE1} \\
		\centering Annese et al. (2018)\cite{Annese2018} &
		\centering 6 AD, 6 Controls, 6 PD &
		\centering RNA-Seq &
		whole transcriptome profiling of hippocampus, temporal and frontal regions of late-onset Alzheimer’s disease (LOAD) vs control with expression of both non-coding and coding protein RNA transcriptsidentified 2,122 differentially expressed genes (after FDR \textless 0.05), of which 2,075 differentially expressed protein-coding genes in AD vs control, 789 upregulated and 1,286 downregulated \\
		\centering Raj et al. \newline (2018)\cite{Raj2018} &
		\centering 268 AD, 182 Control \newline Dorsolateral prefrontal cortex &
		\centering RNA-Seq &
		84 genes identified as differentially spliced and of which 11 were characterised with differential expression. 
		TWAS for 4,756 genes and 15,013 differentially spliced introns in 21 genes with expression changes associated with AD, including \textit{CR1,PTK2B, AP2A1, AP2A2, MAP1B}
		\\
		Maths et al.\\(2019) &
		&
		&
		Single nucleus transcriptomes from prefrontal cortex of 24 individuals with no pathology vs 24 individuals with AD pathology with transcriptional profiling of the different cell types, observing robust cell-specific differental gene expression relating to myelination particularly at early stages of the disease. DEGs in neurons were strongly downregulated whereas DEGs in oligodendrocytes, astrocytes and microglia were mostly uupregulated \\
		\centering Adusumalli et al. (2019) \cite{Adusumalli2019} &
		\centering Human: 42AD, 38 Control (RNA-Seq); 10AD, 10 Control (Mass-Spec)Frontal cortex &
		\centering RNA-Seq, Mass Spectrometry &
		1,136 differential IR events at 781 genes (inc. BIN1, MAPT) between the control and AD, of which IR genes were highly enriched in mRNA export and splicingHigh GC content at retained introns cmopared non-IR introns, indicating changes in DNA methlation may contribute to differential IR Protein expression of differential IR genes significantly different between AD and control \\
		\centering Han et al. (2019) \cite{Han2019} &
		\centering Human: 24 AD vs 50 ControlHippocampus &
		\centering RNA-Seq &
		3 exon skipping events in RELN and NOS1, associated with AD, resulting in truncated protein with lost of functional domains. One SNP was identified adjacent to skipped exon of RELN and located within splicing regulatory element. \\
		Li et al. 2021 \cite{Li2021} &
		Human - Temporal Cortex84 AD, 80 Control128 mouse across 3,6,9,12,20 monthsMouse Model - CRND8, APPPS1 &
		RNA-Seq, Mass Spectrometry &
		Evidence of upregulated intron-retained levels in AD vs controls, including in AD-associated genes such as BACE1, BIN1, PICALM. However, no differential gene expression observed suggesting a compensatory mechanism of altered expression of intron-retained isoforms. HMBOX1, a transcription factor involved in innate immune response, was most associated differentially expressied intron with tau pathology. Transcriptomic-proteomic integrative analysis revealed increased IR was associated with reduced protein expression, possibly due to nonsenese mediated decay \\* \bottomrule
	\end{longtable}
\end{landscape}
\end{changemargin}
...
\subsection{Application of long-read sequencing to AD}
