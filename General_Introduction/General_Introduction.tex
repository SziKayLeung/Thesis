...
Unlike FAD, LOAD does not follow a Mendelian inheritance pattern. Nonetheless, a relatively high heritability rate of 50-80\% \cite{Gatz2006} has been reported, indicating that there is still a large genetic predisposition for developing AD. Indeed over recent years, emergence of genome-wide association studies (GWAS\nomenclature{GWAS}{Genome-wide association studies}) and subsequent meta-analyses \cite{Bellenguez2020,Naj2020,Kunkle2019,Jansen2019,Lambert2013,Naj2011,Hollingworth2011,Harold2009,Lambert2009,Bertram2008} have facilitated the identification of multiple genetic loci associated with an increased risk of developing LOAD. These genetic loci are typically changes or variants of single DNA base-pair (single-nucleotide polymorphisms – SNPs\nomenclature{SNP}{Single Nucleotide Polymorphism}) that are more commonly found in individuals with LOAD than without. 

The strongest risk factor for LOAD is \textit{APOE} gene $\epsilon$4 allele\cite{Lambert2013}, which encodes for the cholesterol transporter apolipoprotein E (ApoE) - a key regulator of lipid homeostasis by mediating lipid transport between astrocytes and neurons, a process critical for synaptic function and maintenance\cite{DH2001}. While the $\epsilon$4 allele is estimated to occur in \textasciitilde 15\% of the general population, it is observed in 40\% of patients with AD\cite{Farrer1997,alzgene2010}. Harbouring one APOE$\epsilon$4 allele further increases the risk of developing LOAD by 3-4x, while harbouring two $\epsilon$4 alleles increases the risk by 15x \cite{Farrer1997}. Conversely, the $\epsilon$2 allele confers a neuroprotective effect against AD \cite{Nagy1995,EH1994}. 

However, with the exception of \textit{APOE}, the 42 genetics variants identified in GWAS were either common but lowly penetrant (such as those annotated to \textit{CLU, PICALM, CR1}) or high penetrantly but rare (those annotated to \textit{TREM2}) and collectively only contributed modestly to the risk of developing AD, highlighting the polygenic nature of AD (\cref{fig:AD_gwas}). While the molecular mechanisms behind these variants currently remain poorly understood, many of these AD-associated genes were enriched in specific biological pathways, such as endocytosis, immune system and inflammatory response, and lipid metabolism (described in \cref{aetiologyAD}). Furthermore, the vast majority of variants were annotated to noncoding regulatory regions of the genome, and were enriched in open chromatin regions that promote transcription such as enhancers\cite{Kikuchi2019} (short DNA sequences containing specific motifs for binding of transcription factors). Recent transcriptomic single-cell studies have further demonstrated that AD-associated non-coding SNPs were largely confined to microglia enhancers with cell-type specific regulation of gene expression\cite{Tansey2018,Nott2019,Young2021,Novikova2021}. Efforts to better understand the expression changes in AD by transcriptome profiling of affected brain regions would thus be crucial to reveal insights into the molecular mechanisms that underlie AD risk variants.

\clearpage
\begin{landscape}
	\begin{figure}[!htp]
		\centering
		\includegraphics[page=11,trim={0 17cm 0cm 1cm},clip, scale = 1.2]{Introduction_Figures.pdf}
		\captionsetup{width=1.6\textwidth,singlelinecheck=off}
		\caption[Genetic landscape of AD]%
		{\textbf{Genetic landscape of AD}: Shown are the genes implicated in AD from the causative EOAD genes (\textit{APP, PSEN1, PSEN2}) identified from early family studies, to the genes identified from GWAS studies annotated with either common but lowly penetrant or highly penetrant but rare variants. Variant penetrance, or effect size, is measured with the odds ratio (OR) with a larger odds ratio referring to a larger effect size. Variants conferring a protective effect are denoted in orange, and those with a negative effect in blue. Figure was taken from DeRojas et al. (2021)\cite{DeRojas2021}. GWAS - Genome-Wide Association Study, OR - odds ratio
		}
		\label{fig:AD_gwas}
	\end{figure}
\end{landscape}
...
\begin{figure}[!htp]
	\centering
	\includegraphics[page=9,trim={0 16.5cm 0cm 0},clip, scale = 0.7]{Introduction_Figures.pdf}
	\captionsetup{width=0.95\textwidth,singlelinecheck=off}
...
\begin{landscape}
	...
	\begin{longtable}[c]{p{3cm}p{4cm}p{3cm}p{16cm}}
		...
		{\textbf{Transcriptomic and proteomic studies in AD human post-mortem brain tissues in revealing missplicing as a widespead hallmark of AD}. \textsuperscript{a}RNA-Seq data from ROSMAP dataset. AS - Alternative Splicing, NMD - Nonsense Mediated Decay, RBP - RNA binding protein, TSS - Transcription Start Site}
...
\begin{landscape}
	\begin{figure}[htp]
		\centering
		\includegraphics[page=10,trim={0 19cm 2cm 1cm},clip, scale = 1]{Introduction_Figures.pdf}
...
The consequent generation of longer reads provided unprecedented ability to sequence entire or new entire lengths of transcripts from 5’ end to polyA tail, relinquishing the need for transcriptome assembly and resolving splicing junctions (\cref{fig:longread_benefits}\textbf{d}). Allowing greater accuracy at transcript identification, an increasing number of studies have used such technologies to characterise isoform diversity and splicing with unprecedented success (Table X). Generally in comparison with RNA-Seq, Iso-Seq encapsulates longer transcripts, identifies novel gene locus, and correction of gene model. "Long transcript reads provide better support and higher accuracy in splice junctions than short reads, when these reads are aligned back to the genome. Thus gene models predicted from long reads yield more accurate exon/intron structure and can merge two or more misannotated adjacent genes."

\begin{figure}[htp]
	\centering
	\includegraphics[page=12,trim={0 10cm 0 0},clip, scale = 0.8]{Introduction_Figures.pdf}
	\captionsetup{width=0.95\textwidth,singlelinecheck=off}
	\caption[Benefits of using long-reads for transcript assembly]%
	{\textbf{Long-read sequencing apporaches capitalise on real-time sequencing of templates, generating long-reads that span the entire transcript length}: Shown is an overview of the three main sequencing approaches for RNA-Seq, after library preparation: 		
	\begin{enumerate}[label=\textbf{\alph*)}]
		\item short-read RNA-Seq on the illumina platform: cDNA is clustered on flowcell for a “sequence-by-synthesis” fashion by the complementary binding of fluorescently-labelled nucleotides at each sequencing cycle, followed by washing, scanning and removing of labels 
		\item long-read RNA-Seq on the PacBio platform: cDNA is loaded onto a sequencing chip with an immbolised polymerase, and is sequenced in real-time by the incoporation of fluorescently-labelled nucleotides
...
\begin{changemargin}{1.5cm}
	...
	\begin{landscape}
		...
		\setlength\tabcolsep{2pt} %reduced margin size in table
		\renewcommand{\arraystretch}{1}
		\begin{longtable}[c]{p{3cm}p{4cm}p{19cm}}
			\caption[Long-read sequencing using PacBio Iso-Seq]%		
			{\textbf{Long-read sequencing} PacBio Iso-Seq - Pacific Bioscience's Isoform Sequencing}
			...
			\endlastfoot
			%		
			\centering Au et al. (2012)\cite{Au2012} &
			\centering Human cerebellum &
			\tabitem Correction of raw PacBio long reads sequenced on human cerebellum transcriptome (RSII) with short reads using developed algorithm (LSC), reducing rate by three fold \\
			\hdashline[0.5pt/5pt]
			
			\centering Sharon et al. (2013)\cite{Sharon2013} &
			\centering 20 Human tissues &
			\tabitem 1st paper to demonstrate feasibility to sequence RNA transcripts up to 1.5kb from human tissues, identifying 14,000 genes with estimated 10\% of reads mapping to unannotated transcripts.  \\
			\hdashline[0.5pt/5pt] 
			
			\centering Au et al. (2013)\cite{Au2013} &
			\centering Human embryonic stem cell transcriptome &
			\tabitem 1st paper to characterise stem cells using long-read sequencing, in combination with short-read RNA-Seq, detecting 8,084 known full-length isoforms and 1,800 novel isoforms from statistical inference \\
			\hdashline[0.5pt/5pt]
			
			\centering Tilgner et al. (2014) \cite{Tilgner2014} &
			\centering Human Lymphoblastoid &
			\tabitem Iso-Seq of 3 family members (2 parents, 1 daughter), identifying reads for genes up to 3kb length, novel isoforms, and determining SNVs with the potential to connect to RNA haplotypes (i.e. which allele the RNA molecule originated from)  \\
			\hdashline[0.5pt/5pt]
			
			\centering Treutlein et al. (2014)\cite{Treutlein2014} &
			\centering Mouse prefrontal cortex &
			\tabitem Enrichment of Neurexin identified 2,000 isoforms across \textit{Nrxn1a, Nrxn3a, Nrxn1b-3b} and novel variantts with novel splice sites resulting in partial or complete deletion of domain structure \\
			\hdashline[0.5pt/5pt]
			
			\centering Schreiner et al. (2014)\cite{Schreiner2014} &
			\centering Mouse prefrontal cortex &
			\tabitem Complementary paper to Treutlein et al. (2014), achieving even deeper sequencing coverage of Neurexin (specifically Nrxna) by using a similar targeted sequencing approach, but amplification with plasmid-based library \\
			\hdashline[0.5pt/5pt]
			
			\centering Linnarssson et al. (2017) &
			\centering FXTAS  &
			\tabitem First paper to use long read, single-cell sequencing (Iso-Seq) in mouse cells, enabled by the use of unique molecular identifiers. High isoform diveristy observed within a single cell (6 single cells: 2 vascular and leptomeningeal cells (VLMCs), and 4 cells represented stages of oligodendrocyte maturation), with up to 1000 distinct isoforms mapped to 700 genes identified, and with small overlap of isoforms between cells. Isoform diveristy was predominatly driven by alterantive TSS and TSS, and was positively correlated to gene expression. Exon junctions in non-coding isoforms were furthermore more variable than exon junctions in coding isoforms, indicating a selection on splice donor and acceptor usage.  \\
			\hdashline[0.5pt/5pt]	
			
			\centering Tseng et al. (2017) \cite{Tseng2017} &
			\centering FXTAS  &
			\tabitem PacBio Targeted Iso-Seq \\
			\hdashline[0.5pt/5pt]
			
			
			\centering Gupta et al. (2018) \cite{Gupta2018} &
			\centering Breast cancer cell line  &
			\tabitem First paper to perform long-read sequencing of > 5000 single cells (rather than previously in < 10 cells; of note RNA-sequencing of single cells up to several hundreds have been performed back in 2013) using developed strategy: single-cell isoform RNA-Seq (ScISOr-Seq) - combined gene expression from RNA-Sequencing of single cells isolated from 10X Genomics and isoform identification from Iso-Seq. ONT nanopore sequencing was also tested with this approach, a lower number of reads were detected per flow cell  (50,000  vs 5.2Million CCS reads) due to higher error rate from incorrect reading of homopolymers - more PCR cycles would be required resulting in artifacts. Applying this approach, and in being able to accurately sequence different cell types including microglia, astrocytes, and neurons - they identified cell-specific isoform expression of BIN1. There was evidence of skipping of Bin1 alternative exons (A1, A2-A6) in microglial, and in most of astryocyte and oligodendroycte but not in neuronal cell-types.  \\
			\hdashline[0.5pt/5pt]
			
			\centering Nattestad et al. (2018) \cite{Nattestad2018} &
			\centering Breast cancer cell line  &
			\tabitem PacBio Iso-Seq \\
			\hdashline[0.5pt/5pt]
			
			\centering Dougherty et al. (2018) \cite{Dougherty2018} &
			\centering Human brain  &
			\tabitem Using targeted hybridisation capture (initial verison of adopted targeted approach) and Iong-read sequencing (Iso-Seq) to resolve structurally complex regions involving segments of high sequence identity (segmental duplications - important for species evolution). Targeted sequencing of 19 gene families (previously identifed to be duplicated from deep sequencing of archaic and contemporary hominid genomes) found majority of duplicated genes to have undergo significant mRNA processing events (novel sites for transcription initation, splicing, fusion and polyadenylation), with potential to encode for truncated proteins with different functions.  \\
			\hdashline[0.5pt/5pt]
			
			\centering Tian et al. (2019) \cite{Tian2019} &
			\centering FXTAS  &
			\tabitem PacBio Targeted Iso-Seq \\
			\hdashline[0.5pt/5pt]
			...
			\centering Mays et al. (2019) \cite{Dougherty2018} &
			\centering Human brain Marrow  &
			\tabitem Isoform sequencing of progenitor (lineage negative) and differentiated (lineage positive) cells from human bone marrow tissue, followed by mass spectrometric validation by using long-read data as reference. Of note, paper highlights the power of long-read vs short-read RNA-Seq to characterise transcriptome landscape: RNA-Seq's power to identify number of isoforms per gene was saturated at 4 (regardless of number of exons, and sequencing depth even if increased to 100million), whereas Iso-Seq identifed >10 isoforms (12-36 isoforms); RNA-Seq's ability to recover transcripts with >4 exons drastically drops in comparison to Iso-Seq; RNA-Seq unable to identify transcripts for paralogous genes with conserved sequences \\
			\hdashline[0.5pt/5pt]
			
			\centering Lian et al. (2019) \cite{Dougherty2018} &
			\centering Breast Cancer  &
			\tabitem Using Isoform sequencing to characterise splicing in human breast cancer cells associated with drug resistance (paclitaxel), identifying over 350 differentiated isoforms and validation of a differentially-expressed novel isoform of BAK1 using PCR \\
			\hdashline[0.5pt/5pt]
		...
		\begin{longtable}[c]{p{3cm}p{4cm}p{19cm}}
			...
			\centering Human cerebellum &
			\tabitem First paper to use and show feasibility of Oxford nanopore sequencing to characterise the connectivity i.e if able to sequence full length transcripts that, of distantly located exons in genes with complex splicing patterns (notably, DSCAM in drosophila); Authors concluded advantages to PacBio in that large amounts of cDNA needed, many cycles of PCR resulting in template switching (in paper, optimised number of PCR cycles with spike-ins to reduce template switching --> cycle 25). Of note, ONT technology only alowed targeted sequencing due to low coverage from whole transcriptome sequencing and thereby not able ot sequence lowly expressed transcripts.   \\
			\hdashline[0.5pt/5pt]

			\centering Byrne et al. 2017 \cite{Byrne2017} &
			\centering 84 AD, 80 Controls \newline Temporal cortex &
			\tabitem First paper to show the feasibility of ONT (MinION) to accurately characterise isoforms at single-cell level (with identiifcation of thousands of novel TSS and TTS), in mouse B1a cells. Achieved very good correlation with illumina data within the same cells, but low correlation across different cells, indicating identification of cell-to-cell variability. Identiifed around 170 genes with complex splicing events, 55 of which were differentially expresed using in-house Mandalorian pipeline, which genes included B-cell specific surface receptors, implicating the unique expression of different isoforms across cells (heterogenity between single cells). Of note, UMIs were not used, due to the need for longer UMIs to overcome ONT error rate and subsequent introduction of unwanted PCR artifacts. Instead, cells were sorted using FACS, cDNA synthesis of single cells, and then sequencing of individual cell libraries on MinION.  Nonetheless, despite the ONT error rate which only allowed identification of TSS/TES to a 20bp pin, still impressive feat to characterise transcriptome landscape at a single cell level. \\
			
			\centering DeRoeck et al. 2017  &
			\centering 84 AD, 80 Controls \newline Temporal cortex &
			\tabitem RNA-Seq (n = 928 EOAD, 980 control, anterior cingulate cortex) and validation with targeted ONT sequencing (n = 1 EOAD anterior cingulate cortex) identified 10 novel AD-specific PTC mutations and validated 7 previously reported mutations of ABCA7( (p.Met370fs,  p.Glu709fs, p.Leu1403fs, p.Trp1336*, c.3577 + 1G > C, c.67-1G>A,), which were 5x more frequent in EOAD patients. Interstingly, for all PTC mutations observed in controls, a potential rescue mechanism was present.  \\
			
			\centering De Jong et al. 2017  &
			\centering 84 AD, 80 Controls \newline Temporal cortex &
			\tabitem Another paper to highlight complexity of alternative splicing at a gene-specific level, with targeted PCR followed by ONT sequencing of BRCA1 gene. Note, this paper was not quantitative in that long-range PCR biased amplifcation of short transcripts,  (<4kB), and many of the novel isoforms had <3 MinION reads (> 10\% error rate).   \\
			
			\centering Ebbert et al. (2018) \cite{Ebbert2018} &
			\centering NA  &
			\tabitem NA \\
			\hdashline[0.5pt/5pt]
			...
			\centering 84 AD, 80 Controls \newline Temporal cortex &
			\tabitem Following on from Byrne et al. (2017) to improve nanopore accuracy by circularising input cDNA and generating a CCS read similar to PacBio (R2C2 - Rolling Circle Amplification to Concatemeric Consensus), achieving a similar accuracy as PacBio Iso-Seq with a mimum of 10 subreads from R2C2 approach (316,000 full-length cDNA reads at 94\% accuracy vs 200,000 reads at close to 99\%). Using SIRV, R2C2 able to accurately identify splice sites (92\%) compared to ONT 1D raw reads (80\%) and similar to PacBio CCS reads (97\%). Furthermore, R2C2 able to accurately demultiplex reads based on 7-8nt barcodes, enabling sequencing of 96 B-cells (FACS sorted and amplifed from 1 healthy donor), with accurate quantification of gene expression (correlated well with RNA-Seq) and identification of cell-specific isoforms, including B-cell specific surface receptors in human \\

			
			\centering Garalde et al. 2018 \cite{Garalde2018} &
			\centering 84 AD, 80 Controls \newline Temporal cortex &
			\tabitem First paper, published by ONT, to directly sequence RNA achieving good coverage of full-length transcripts in yeast transcriptome (note 2.8M reads using direct RNA, 5.7M reads using ONT cDNA and 572M reads using RNA-Seq, so relatively lower output compared to cDNA). Using SIRV, able to accurately identify splice variants from direct RNA with no missing or novel exons. Lastly, was able to rudimentally discrimate RNA modification (m6-A, 5-mC) using trained datasets, however needs further optimisation. Multiple advantages to be able to sequence direct-RNA, and paper illustrates the potential to do this, however still require many optimisation with regards to nanopore (using the same pore in cDNA sequencing), motor speed, and bioinformatics tools.   \\
			
			\centering Zeng et al. (2018) \cite{Zeng2019} &
			\centering NA  &
			\tabitem NA \\
			\hdashline[0.5pt/5pt]
			...
			\centering 84 AD, 80 Controls \newline Temporal cortex &
			\tabitem Direct RNA-Sequencing of human transcriptome (human cell line GM12878) using ONT MinION to assess 3' polyA tail length (variation between mitochondrial and nuclear), base modifications and haplotypes. Of note, while was able to assess the different modifications, error rate still very high (14\% per read base called error) and 5' truncation (from translocation of nanopore) making it difficult to ascertain TSS.   \\
			
			\centering Deng et al. (2019) \cite{Deng2019} &
			\centering NA  &
			\tabitem NA \\
			\hdashline[0.5pt/5pt] \\
			
			\centering Sone et al. (2019) \cite{Sone2019} &
			\centering NA  &
			\tabitem NA \\
			\hdashline[0.5pt/5pt]
			...
			\centering Clark et al. 2020 \cite{Workman2019a} &
			\centering 84 AD, 80 Controls \newline Temporal cortex &
			\tabitem First paper to use long-range targeted PCR followed by long read-sequencing (ONT) in human post-mortem brain tissue (across 3 brain regions, cerebellum, striatum, dorsolateral prefrontal cortex), to characterise full coding sequence of CACNA1C gene, implicated in multiple psychiatric disorders. Demonstrated power of long read seuqnecing for annotation and characterisation of alternatively-spliced transcripts with 5x more annotated transcripts and a singular major isoform in most regions  \\
			...
			\centering Kucukali et al. 2021 \cite{Kucukali2021} &
			\centering NA &
			\tabitem NA\\
			
			\centering Reyes et al. 2021 \cite{Reyes2021} &
			\centering NA &
			\tabitem NA\\* \bottomrule
\end{longtable}
\end{landscape}
\end{changemargin}
...
\begin{changemargin}{1.5cm}
	\begin{landscape}
		...
		\begin{longtable}[c]{p{3cm}p{4cm}p{3cm}p{16cm}}
			...
			\caption{Transcriptomic and proteomic studies in AD human post-mortem brain tissues and mouse models in revealing missplicing as a widespead hallmark of AD. AS - Alternative Splicing, RBP - RNA binding protein, TSS - Transcription Start Site}\\
			\toprule
			\multicolumn{1}{c}{References} &
			\multicolumn{1}{c}{Samples and Tissue} &
			\multicolumn{1}{c}{Method} &
