\documentclass[Project_Development.tex]{subfiles}

\section{Iso-Seq Protocol}
%\etocsetnexttocdepth{}
...
\begin{itemize}
	\item Double stranded DNA sample (dsDNA) generated from cDNA synthesis of extracted RNA 
	\item Minimum freeze thaw cycles 
	\item No exposure to high temperature (>65) or pH extremes (<6, >9), 
	\item 1.8 - 2 OD260/280, and 2.0 - 2.2 OD260/230 
...
\begin{enumerate}
	...
	\begin{itemize}
		\item Ensure exact concentration particularly for 0.4X ampure beads - too high concentration would result in retainment of undesired short inserts, too low concentration would result in significant yield loss 
	\end{itemize} 
	...
	\end{itemize}
	\item Elute with specified amount of PacBio Elution Buffer (differs pending on the part of the protocol)
...
\begin{enumerate}
	\item Set up and label the required number of Qubit assay tubes (0.5mL) for samples and 2 samples. 
	\begin{itemize}
...
\begin{enumerate}
	\item For each sample, add 200ng of RNA with 1$\mu$L of barcoded/non-barcoded polyT primer in a micro centrifuge on ice (Table X), mix and spin briefly
	...
	\item During incubation, prepare PCR reaction mix by combining the following reagents in Table X in the order shown. Scale reagent volumes accordingly to the number of samples prepared
	\begin{itemize}
	...
	\item Add 90$\mu$L of PacBio Elution Buffer (EB) to each RNA reaction tubes: diluted first-strand cDNA (Table \ref{tab:cdna_synthesis})
\end{enumerate}
...
\subsection{PCR Cycle Optimisation} 
\begin{enumerate}
	...
	\end{itemize} 
	\item Determine the number of optimum PCR cycles to generate a sufficient amount of ds-cDNA without the risk of over-amplification (Section X)
\end{enumerate} 
...
\subsection{Large-Scale PCR} 
\begin{enumerate}
	\item Set up and label 16 micro centrifuge tubes for each sample 
	\item Prepare a PCR reaction mix for each sample in 1.5mL LoBind eppendorf (Table \ref{tab:large_scale_pcr})
	...
	\item Cycle the reaction with the conditions outlined in Table \ref{tab:PCR_conditions_large_scale_pcr} 
\end{enumerate}
\
\begin{table}[ht]
	\centering
...
\subsubsection{Fraction 1: 2nd purification}
\begin{enumerate}
...
\subsubsection{Repair DNA Damage and Ends}
\begin{enumerate}
